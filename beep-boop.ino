...
void setup() {
    ...
    Particle.publish("beepDeviceState","beepStandby");
    Particle.publish("beepDeviceLight","off");

    // Gets update when boop device updates state
    Particle.subscribe("boopDeviceState", stateHandler);
    Particle.subscribe("boopDeviceLight", lightHandler);
...
void loop() {
    ...
    if (screenState == 1 || !textLoop) {
        ...
        if (lastScreenState != screenState && textLoop) {
            Particle.publish("beepDeviceState","beepSent");
        ...
        if (lastScreenState != screenState) {
            lastScreenState = screenState;
            Particle.publish("beepDeviceState","beepDelivered");
            display.setTextSize(1);
        }

        display.setCursor(0, 56);
        display.print("Beep sent...");
        ...
        if (lastScreenState != screenState) {
            lastScreenState = screenState;
            Particle.publish("beepDeviceState","boopReceved");
        ...
        // Boop has been read
        lastScreenState = screenState;
        Particle.publish("beepDeviceState","boopRead");
    ...
    if(wasPressed(lightButton, &lightButtonLast)) {
        if (lightState) {
            ...
            lightState = 1;
            Particle.publish("beepDeviceLight","on");
        }
    }
...
void stateHandler(const char *event, const char *data) {
    if (!strcmp(data,"boopSent")) {
        // Incoming boop
        screenState = 3;
    } else if (!strcmp(data,"beepRead")) {
        // Our beep has been read
        screenState = 0;
    } else if (!strcmp(data,"beepReceved")) {
        // Our beep has been noted but not read
        screenState = 2;
    }
}


// light Handler is called when the cloud tells us that the boop's light is toggled
void lightHandler(const char *event, const char *data) {
    if (!strcmp(data,"on")) {
        // Boop light was turned on
        lightState = 1;
    } else if (!strcmp(data,"off")) {
        // Boop light was turned off
        lightState = 0;
    }
}
